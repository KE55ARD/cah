...
if("$newemail" == "$confemail") then
  sql -d cah 'UPDATE users SET email="$newemail" WHERE session="$HTTP_SESSION"'
  echo "Location: index.cgi?SUCCESS=Email%20changed%20successfully"
else
  echo "Location: editemail.cgi?ERROR=Emails%20do%20not%20match."
endif
echo ""
